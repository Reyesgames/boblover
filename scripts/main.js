...
'use strict'; {
    ...
    class C3BannerAd {
        ...
        Release() {
            if (this._remove_load_listeners) this._remove_load_listeners();
            this._remove_load_listeners = null;
            this._load_settled_promise = null;
            this._load_settled_promise_resolver = null;
            this._ad = null
        ...
        Load(adPersonalization, resolver) {
            ...
            const options = {
                "adUnitId": C3AdUtils.GetAdUnit(this._adUnitId,
                    "banner"),
                "size": this._size,
    ...
    function _AddLoadListeners(resolver) {
        const onLoad = event => {
            ...
            if (this._load_settled_promise_resolver) this._load_settled_promise_resolver()
        };
...
'use strict'; {
    ...
    class C3InterstitialAd {
        ...
        HandleOnLoadedResult(result, resolver) {
            if (result) C3AdUtils.Success(resolver, "interstitial ad load");
            else C3AdUtils.Failure(resolver, "interstitial ad failed to load")
        }
        async Show(resolver) {
            if (!this._ad) return Failure(resolver,
                "interstitial ad not created");
...
'use strict'; {
    ...
    class C3RewardedAd {
        ...
        HandleOnLoadedResult(result, resolver) {
            if (result) C3AdUtils.Success(resolver, "rewarded ad load");
            else C3AdUtils.Failure(resolver, "rewarded ad failed to load")
        }
        async Show(resolver) {
...
'use strict'; {
    ...
    class C3RewardedInterstitialAd {
        ...
        HandleOnLoadedResult(result, resolver) {
            if (result) C3AdUtils.Success(resolver, "rewarded interstitial ad load");
            else C3AdUtils.Failure(resolver, "rewarded interstitial ad failed to load")
        }
        async Show(resolver) {
    ...
    function _AddShowListeners(resolver) {
        ...
        const onShowFail = event => {
            ...
            document.removeEventListener(Events.SHOW, onShow);
            document.removeEventListener(Events.SHOW_FAIL,
                onShowFail);
            document.removeEventListener(Events.REWARD, onReward);
            document.removeEventListener(Events.DISMISS, onDismiss)
        };
        ...
        document.addEventListener(Events.SHOW_FAIL, onShowFail);
        document.addEventListener(Events.REWARD, onReward);
        document.addEventListener(Events.DISMISS,
            onDismiss)
    }
...
'use strict'; {
    ...
    addHandler("CreateBannerAdvert", async (id, size, debug, position, reload, fn) => {
    ...
    addHandler("CreateInterstitialAdvert", async (id, debug, fn) => {
        if (interstitialAd) {
            if (interstitialAd.IsShowing()) return C3AdUtils.Success(fn, "interstitial ad already showing");
    ...
    addHandler("CreateVideoAdvert", async (id, debug, fn) => {
        if (videoAd || rewardedAd) {
            if (rewardedAd) {
                if (rewardedAd.IsShowing()) return C3AdUtils.Success(fn, "rewarded ad already showing");
            ...
            if (videoAd) {
                if (videoAd.IsShowing()) return C3AdUtils.Success(fn, "rewarded ad already showing");
    ...
    addHandler("CreateRewardedInterstitialAdvert", async (id, debug, fn) => {
        if (rewardedInterstitialAd) {
            if (rewardedInterstitialAd.IsShowing()) return C3AdUtils.Success(fn, "rewarded interstitial ad already showing");
            if (rewardedInterstitialAd.IsLoading()) {
                const ret =
                    await rewardedInterstitialAd.GetLoadPromise();
    ...
    addHandler("RequestConsent", async fn => {
        api["Configure"](C3AdUtils.ConfigurationOptions.id, C3AdUtils.ConfigurationOptions.pubID,
            C3AdUtils.ConfigurationOptions.privacyPolicy, C3AdUtils.ConfigurationOptions.showFree, "always", C3AdUtils.ConfigurationOptions.debug, C3AdUtils.ConfigurationOptions.debugLocation, false, fn)
    });
    ...
    addHandler("TagForChildDirectedTreatment",
        async (label, fn) => {
            if (!await C3AdUtils.StartAdmobPlus(fn)) return;
            try {
